...
import { CheckCircle, Download, Package, ChevronDown, ChevronUp, HelpCircle, Info, Star, Shield, Zap, List } from "lucide-react";
...
export const PluginDetailsModal = ({ isOpen, onClose, plugin, onPurchase }: PluginDetailsModalProps) => {
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    description: true, features: true, bullets: false, benefits: false, requirements: false, faq: false,
  });
  ...
  const sections = [
    {
      id: "description", label: "About This Product", icon: <Info className="w-5 h-5" />,
      content: plugin.description ? <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{plugin.description}</p> : <p className="text-sm text-muted-foreground italic">No description available.</p>,
    },
    {
      id: "bullets", label: "Key Features", icon: <List className="w-5 h-5" />,
      content: (plugin.bullets || []).length > 0 ? (
        <div className="grid gap-2">
          {(plugin.bullets || []).map((b, i) => (
    ...
    {
      id: "requirements", label: "Requirements", icon: <Shield className="w-5 h-5" />,
        ...
        <div className="space-y-1.5 p-3 rounded-lg bg-muted/50 border border-border/50">
          {(plugin.requirements || []).map((req, i) => (
            <p key={i} className="text-sm text-muted-foreground flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary/60 flex-shrink-0" />{req}
            </p>
          ))}
        </div>
      ) : null,
    },
    {
      ...
      content: (plugin.faq || []).length > 0 ? (
        <div className="space-y-3">
          {(plugin.faq || []).map((f: FaqItem, i: number) => (
            <div key={i} className="p-3 rounded-lg bg-muted/30 border border-border/30">
              <p className="font-medium text-sm mb-1">Q: {f.question}</p>
          ...
          <div className="p-4 sm:p-6">
            ...
            {allImages.length > 0 && (
              <div className="mb-6">
                <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-muted/30 border border-border/30">
                  <img src={allImages[activeImage]} alt={plugin.name} className="w-full h-full object-cover" />
                </div>
                {allImages.length > 1 && (
                  <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
                    {allImages.map((img, i) => (
                      <button key={i} onClick={() => setActiveImage(i)} className={`w-16 h-12 rounded-lg overflow-hidden border-2 flex-shrink-0 transition-all ${i === activeImage ? "border-primary shadow-lg" : "border-border/30 opacity-60 hover:opacity-100"}`}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6 p-4 rounded-xl bg-muted/30 border border-border/30">
              <div className="flex items-center gap-3">
                {plugin.version && <Badge variant="secondary" className="text-xs"><Package className="w-3 h-3 mr-1" />v{plugin.version}</Badge>}
                {plugin.category && <Badge variant="outline" className="text-xs">{plugin.category}</Badge>}
                {(plugin.tags || []).slice(0, 3).map((tag, i) => <Badge key={i} variant="outline" className="text-xs hidden sm:inline-flex">{tag}</Badge>)}
              </div>
              <div className="flex items-center gap-2">
                {plugin.originalPrice && <span className="text-lg line-through text-muted-foreground">${plugin.originalPrice}</span>}
            ...
            <div className="space-y-3 mb-6">
              {sections.map((section) => (
                <div key={section.id} className="rounded-xl border border-border/30 overflow-hidden">
                  <button onClick={() => toggleSection(section.id)} className="flex items-center justify-between w-full p-3 sm:p-4 hover:bg-muted/30 transition-colors">
            ...
            <div className="flex gap-3 sticky bottom-0 bg-background/95 backdrop-blur-xl pt-4 pb-2 border-t border-border/30 -mx-4 sm:-mx-6 px-4 sm:px-6">
              ...
              <Button onClick={onPurchase} className="flex-1 glass-button"><Download className="w-4 h-4 mr-2" />Buy Now - ${plugin.price}</Button>
            </div>
          </div>
        </ScrollArea>
      </DialogContent>
