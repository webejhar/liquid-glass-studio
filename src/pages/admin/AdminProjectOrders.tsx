...
export default function AdminProjectOrders() {
  ...
  useEffect(() => {
    loadProjects();
    const channel = supabase.channel('admin-projects').on('postgres_changes', { event: '*', schema: 'public', table: 'projects' }, loadProjects).subscribe();
  ...
  const loadProjects = async () => {
    try {
      ...
      const projectsWithUsers = await Promise.all((data || []).map(async (project) => {
  ...
  const handleApprovePayment = async (projectId: string, type: 'advance' | 'final', project: Project) => {
    try {
      ...
      if (type === 'advance') updateData.admin_approved = true;
      else updateData.status = 'completed';
      const { error } = await supabase.from("projects").update(updateData).eq("id", projectId);
      if (error) throw error;
      if (type === 'final' && project.client?.email) {
        await supabase.functions.invoke('send-project-status-email', {
          body: { recipientEmail: project.client.email, recipientName: project.client.name || 'Client', projectTitle: project.project_title, status: 'admin_approved', projectId: project.id, amount: project.final_budget }
        });
      }
  ...
  const handleMarkProviderPaid = async (projectId: string) => {
    try {
      const { error } = await supabase.from("projects").update({ provider_payment_status: 'paid' }).eq("id", projectId);
      if (error) throw error;
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = { pending: "bg-yellow-500/20 text-yellow-400", accepted: "bg-green-500/20 text-green-400", rejected: "bg-red-500/20 text-red-400", in_progress: "bg-blue-500/20 text-blue-400", submitted: "bg-purple-500/20 text-purple-400", completed: "bg-primary/20 text-primary" };
    return <Badge className={styles[status] || "bg-muted"}>{status.replace("_", " ").toUpperCase()}</Badge>;
  };

  const calculatePayments = (project: Project) => {
    if (!project.final_budget) return { advance: 0, final: 0, adminFee: 0, providerAmount: 0, total: 0 };
    ...
    const adminFee = project.final_budget * 0.05;
    const providerAmount = project.final_budget - adminFee;
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 glass-card rounded-xl"><p className="text-muted-foreground">No project orders yet</p></div>
        ) : (
          <div className="space-y-4">
            {projects.map((project, index) => {
              const payments = calculatePayments(project);
              return (
                <motion.div key={project.id} className="glass-card p-4 rounded-xl" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold">{project.project_title}</h3>
                        {getStatusBadge(project.status)}
                        {project.admin_approved && <Badge className="bg-green-500/20 text-green-400">Admin Approved</Badge>}
                      </div>
                      ...
                      {project.final_budget && (
                        <div className="flex flex-wrap gap-4 text-sm">
                          ...
                          <span className={project.advance_paid ? "text-green-400" : "text-yellow-400"}>Advance ({project.advance_percentage}%): ${payments.advance.toFixed(2)} {project.advance_paid ? "✓" : "Pending"}</span>
                          <span className={project.final_paid ? "text-green-400" : "text-muted-foreground"}>Final ({project.final_percentage}%): ${payments.final.toFixed(2)} {project.final_paid ? "✓" : ""}</span>
                        </div>
                      )}
                      {project.status === 'completed' && project.final_budget && (
                        <div className="glass-card p-2 rounded-lg text-sm">
                          <span className="text-primary">Admin Fee (5%): ${payments.adminFee.toFixed(2)}</span> | <span className="text-green-400">Provider Amount: ${payments.providerAmount.toFixed(2)}</span>
                        </div>
                      )}
                      {project.provider_payment_requested && (
                        <div className="glass-card p-2 rounded-lg text-sm">
                          <div className="flex items-center justify-between flex-wrap gap-2">
                            <div>
                              <span className="text-yellow-400 flex items-center gap-1"><Wallet className="w-4 h-4" />Payment Requested via {project.provider_payment_method}</span>
                              <span className="text-muted-foreground ml-5">ID: {project.provider_payment_id}</span>
                            </div>
                            {project.provider_payment_status === 'paid' ? (
                              <Badge className="bg-green-500/20 text-green-400 gap-1"><CheckCircle className="w-3 h-3" />Paid</Badge>
                            ) : (
                              <Button onClick={() => handleMarkProviderPaid(project.id)} size="sm" className="gap-1 bg-green-600 hover:bg-green-700"><Check className="w-4 h-4" />Mark as Paid</Button>
                            )}
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex gap-2 shrink-0 flex-wrap">
                      <Button onClick={() => { setSelectedProject(project); setShowDetails(true); }} size="sm" variant="outline" className="gap-1"><Eye className="w-4 h-4" />View</Button>
                      {project.advance_paid && !project.admin_approved && <Button onClick={() => handleApprovePayment(project.id, 'advance', project)} size="sm" className="gap-1"><Check className="w-4 h-4" />Approve Advance</Button>}
                      {project.status === 'submitted' && project.final_paid && <Button onClick={() => handleApprovePayment(project.id, 'final', project)} size="sm" className="gap-1 bg-green-600 hover:bg-green-700"><Check className="w-4 h-4" />Complete & Pay Provider</Button>}
                    </div>
                  </div>
                </motion.div>
              );
